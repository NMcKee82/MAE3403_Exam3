...
class circuitView():
    ...
    def drawResistor(self, R=resistor(), Model=circuitModel()):
        ...
        center = n1.position.midPt(n2.position) * self.scale
        dist = self.scale * vec.mag()
        width = self.elementFraction / 2 * dist
        height = self.elementFraction * dist

        # Calculate positions for zig-zag pattern
        num_zigs = 4
        segment_length = dist / (2 * num_zigs + 1)
        points = [n1.position]
        for i in range(num_zigs):
            direction = (-1) ** i
            next_point = points[-1] + Position(x=segment_length, y=direction * height)
            points.append(next_point)
            points.append(points[-1] + Position(x=segment_length, y=0))

        points.append(n2.position)

        # Draw the zig-zag pattern
        for j in range(len(points) - 1):
            self.drawALine(StartPt=points[j] * self.scale, EndPt=points[j + 1] * self.scale, Pen=self.penConnect)
        ...
        while i < len(data):
            ...
            if l.find('<node')>=0:
                ...
                while l.lower().find('/resistor') < 0:
                    ...
                    elif l.lower().find('resistance') >= 0:
